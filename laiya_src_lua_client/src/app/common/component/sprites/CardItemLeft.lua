...
local CardItemLeft = class("CardItemLeft", function()
...
function CardItemLeft:ctor(_op,_cardEnum)
...
function CardItemLeft:onCleanup()
    self:unregEvent()
end

--注册事件
function CardItemLeft:regEvent()
    g_msg:reg(self.index, g_msgcmd.UI_Setting_Change, handler(self, self.setting))
end

--注销事件
function CardItemLeft:unregEvent()
...
--加载ccbi
function CardItemLeft:load_ccb()
    self.m_ccbRoot = {}
...
function CardItemLeft:initUI(op,isOffset)
  local tablestyle = g_LocalDB:read("tablestyle")
	self.op = op
    ...
    for k, v in pairs(_child) do
        if k == 1 or k == 2 or k == 3 or k == 4 or k == 6 then
            if v:isVisible() then
              ...
              _height  = _height + v:getContentSize().height*v:getScaleY() - 5
            end
        end
    ...
    local h6 =  _child[6]:getContentSize().height*_child[6]:getScaleY()
    local h1 =  _child[1]:getContentSize().height*_child[1]:getScaleY()
    self.h1  = h1
...
--设置偏移
function CardItemLeft:setCardOffset(_offset)
...
function CardItemLeft:fixMidPong()
    for k, v in pairs(self.child) do
        local y = v:getPositionY()
        v:setPositionY(y - self.h1)
    end
end
