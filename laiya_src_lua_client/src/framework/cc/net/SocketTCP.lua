...
function SocketTCP:connect(__host, __port, __retryConnectWhenFailure)
	...
	if addrifo then
		printTable("SocketTCP:addrifo", addrifo)
	...
	if isipv6_only then
		print("SocketTCP:connect====ipv6")
		self.tcp = socket.tcp6()
	else
		print("SocketTCP:connect====ipv4")
		self.tcp = socket.tcp()
	end
	...
	if not __checkConnect() then
		...
		self:un_connectTimeTickScheduler()
...
function SocketTCP:close( ... )
	...
	self.isConnected = false --dhd修改 close后重连
	self.tcp:close()
	self:un_connectTimeTickScheduler()
...
function SocketTCP:un_connectTimeTickScheduler()
