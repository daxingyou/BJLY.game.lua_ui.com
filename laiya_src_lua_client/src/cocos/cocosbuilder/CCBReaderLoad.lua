    ...
    if nil ~= owner then
        ...
        for i = 1,table.getn(ownerCallbackNames) do
            ...
            if "function" == type(owner[callbackName]) then
                local callback = function(_sender, _event)
                    --修改按钮音效统一方案
                    local soundName = g_audioConfig.btnlist[callbackName] or "btn"
                    print("callbackName-soundName", soundName)
                    if string.len(soundName) > 0 and _event == cc.CONTROL_EVENTTYPE_TOUCH_UP_INSIDE then
                        g_audio.playButtonSound(soundName)
                    end

                    owner[callbackName](_sender, _event)
                end
                proxy:setCallback(callbackNode, callback, ownerCallbackControlEvents[i])
            else
